...
// invalida a sessao do spring security
...
function gerenciaTeclaEnter() {
	$(document).ready(function() {
		...
		$('input[type=text]').keydown(function(e) {
			// Obter o proximo indice do elemento de entrada de texto
			var next_idx = $('input[type=text]').index(this) + 1;

			// Obter o numero de elemento de entrada de texto em um documento html
			var tot_idx = $('body').find('input[type=text]').length;

			// Entra na tecla no codigo ASCII
			if (e.keyCode === 13) {
				if (tot_idx === next_idx)
					// Va para o primeiro elemento de texto
					$('input[type=text]:eq(0)').focus();
				else
					// Va para o elemento de entrada de texto seguinte
...
function invocaApplet(context) {
	   ...
	   //Faz algo com ajax...
		...
		var url = context + "/applet/imprimir.jsp?impressoraImprimir=" + null;// passando null para pegar a padrao
